...
public class Shelter extends LightAbility implements AddonAbility {
  ...
  public void progress() {
    ...
    if (this.shelterType == ShelterType.CLICK) {
      shieldOther();
    } else if (this.shelterType == ShelterType.SHIFT && player.isSneaking()) {
      shieldSelf();
    }
  }
    ...
    if (moveBlast) {
      ...
      if (System.currentTimeMillis() > time + duration) {
        ...
        for (Entity approachingEntity : GeneralMethods.getEntitiesAroundPoint(this.target.getLocation(), othersRadius)) {
          if (approachingEntity instanceof LivingEntity && !approachingEntity.getUniqueId().equals(this.target.getUniqueId()) && !approachingEntity.getUniqueId()
